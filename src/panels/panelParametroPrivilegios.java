 ...
 * @author Family
 */
public class panelParametroPrivilegios extends javax.swing.JPanel {

    ArrayList<Privilegios> colores = new ArrayList();
    int valor = -1;
    String estado = "Activo";
    public boolean agregar=true;
    public boolean editar=true;
    public panelParametroPrivilegios() {
        initComponents();
        reset();
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Administración de Privilegios de Usuarios");
        ...
        nombreTxt.setPhColor(new java.awt.Color(102, 102, 102));
        ...
        nuevoBtn.setToolTipText("Agregar Privilegios de Usuarios");
        ...
        editarBtn.setToolTipText("Editar Privilegio");
    ...
    private void editarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editarBtnActionPerformed
        ...
        if (fila >= 0 && !id.equals("-")) {
            ...
            valor = 1;
            for (Privilegios privilegio : colores) {
                if (idint == privilegio.getId()) {
                    idTxt.setText(privilegio.getId() + "");
                    nombreTxt.setText(privilegio.getNombre());
                    if (privilegio.getEstado().equalsIgnoreCase("activo")) {
                        activoRb.setSelected(true);
                    } else {
    ...
    private void guardarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBtnActionPerformed
        if (validarCamposLlenos() == true) {
            ...
            if (valor == 0) {//nuevo
                ...
                for (Privilegios privilegio : colores) {
                    if (privilegio.getNombre().equalsIgnoreCase(nombre)) {
                        repetido = true;
                    }
                }
                if (repetido == false) {
                    int respuesta = new Privilegios().newPrivilegio(nombre);
                    if (respuesta > 0) {
                        new rojerusan.RSNotifyFade("¡¡Agregado!! ", "Privilegio agregado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                        limpiarCampos();
                ...
                int respuesta = new Privilegios().updatePrivilegio(id, nombre, estado);
                if (respuesta > 0) {
                    new rojerusan.RSNotifyFade("¡¡Actualizado!! ", "Privilegio actualizado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                    limpiarCampos();
                    buscarValorTabla(nombre);
                } else {
    ...
    public void llenarPrivilegios() {
        ...
        colores.clear();
        colores = (ArrayList) new Privilegios().Listar("").clone();
        for (Privilegios privilegio : colores) {
            modelo.addRow(new Object[]{
                privilegio.getId(), privilegio.getNombre(), privilegio.getEstado()
            });
        }
        if (colores.size() < 1) {
            modelo.addRow(new Object[]{
    ...
    public void llenarPrivilegios(String estadox) {
        DefaultTableModel modelo = (DefaultTableModel) tabla.getModel();
        modelo.setRowCount(0);
        colores.clear();
        colores = (ArrayList) new Privilegios().Listar(estadox).clone();
        for (Privilegios privilegio : colores) {
            if(privilegio.getEstado().contains(estadox)){
                modelo.addRow(new Object[]{
                privilegio.getId(), privilegio.getNombre(), privilegio.getEstado()
            });
            }
