 ...
 * @author Family
 */
public class panelParametroCargo extends javax.swing.JPanel {

    ArrayList<Cargos> colores = new ArrayList();
    int valor = -1;
    String estado = "Activo";
    public boolean agregar = true;
    public boolean editar = true;

    public panelParametroCargo() {
        initComponents();
        reset();
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Gestión de Cargos Administrativos");
    ...
    private void editarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editarBtnActionPerformed
        ...
        if (fila >= 0 && !id.equals("-")) {
            ...
            valor = 1;
            for (Cargos cargo : colores) {
                if (idint == cargo.getId()) {
                    idTxt.setText(cargo.getId() + "");
                    nombreTxt.setText(cargo.getNombre());
    ...
    private void guardarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBtnActionPerformed
        if (validarCamposLlenos() == true) {
            ...
            if (valor == 0) {//nuevo
                ...
                for (Cargos cargo : colores) {
                    if (cargo.getNombre().equalsIgnoreCase(nombre)) {
                        repetido = true;
                    }
                }
                if (repetido == false) {
                    int respuesta = new Cargos().newCargo(nombre);
                    if (respuesta > 0) {
                        new rojerusan.RSNotifyFade("¡¡Agregado!! ", "Cargo agregado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                        limpiarCampos();
                ...
                int respuesta = new Cargos().updateCargo(id, nombre, estado);
                if (respuesta > 0) {
                    new rojerusan.RSNotifyFade("¡¡Actualizado!! ", "Cargo administrativo actualizado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                    limpiarCampos();
                    buscarValorTabla(nombre);
                } else {
    ...
    public void llenarCargos() {
        ...
        colores.clear();
        colores = (ArrayList) new Cargos().Listar("").clone();
        for (Cargos cargo : colores) {
            modelo.addRow(new Object[]{
                cargo.getId(), cargo.getNombre(), cargo.getEstado()
            });
        }
        if (colores.size() < 1) {
            modelo.addRow(new Object[]{
    ...
    public void llenarCargos(String estadox) {
        ...
        modelo.setRowCount(0);
        colores.clear();
        colores = (ArrayList) new Cargos().Listar(estadox).clone();
        for (Cargos cargo : colores) {
            if (cargo.getNombre().contains(estadox)) {
                modelo.addRow(new Object[]{
                    cargo.getId(), cargo.getNombre(), cargo.getEstado()
                });
            }
