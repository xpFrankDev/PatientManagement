 ...
 * @author Family
 */
public class panelParametroGruposVulnerables extends javax.swing.JPanel {

    ArrayList<GrupoVulnerable> colores = new ArrayList();
    int valor = -1;
    String estado = "Activo";
    public boolean agregar = true;
    public boolean editar = true;

    public panelParametroGruposVulnerables() {
        initComponents();
        reset();
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Administración de Grupos Vulnerables ");
        ...
        nuevoBtn.setToolTipText("Agregar Grupo Vulnerable");
        ...
        editarBtn.setToolTipText("Editar Grupo Vulnerable");
    ...
    private void editarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editarBtnActionPerformed
        ...
        if (fila >= 0 && !id.equals("-")) {
            ...
            valor = 1;
            for (GrupoVulnerable gv : colores) {
                if (idint == gv.getId()) {
                    idTxt.setText(gv.getId() + "");
                    nombreTxt.setText(gv.getNombre());
                    if (gv.getEstado().equalsIgnoreCase("activo")) {
                        activoRb.setSelected(true);
                    } else {
    ...
    private void guardarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBtnActionPerformed
        if (validarCamposLlenos() == true) {
            ...
            if (valor == 0) {//nuevo
                ...
                if (repetido == false) {
                    int respuesta = new GrupoVulnerable().newGrupoVulnerable(nombre);
                    if (respuesta > 0) {
                        new rojerusan.RSNotifyFade("¡¡Agregado!! ", "Grupo Vulnerable agregado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                        limpiarCampos();
                        buscarValorTabla(nombre);
                        ParametrosPanel.actualizarPams("grupo");
                    } else {
                ...
                int respuesta = new GrupoVulnerable().updateGrupoVulnerable(id, nombre, estado);
                if (respuesta > 0) {
                    new rojerusan.RSNotifyFade("¡¡Actualizado!! ", "Grupo vulnerable actualizado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                    limpiarCampos();
                    buscarValorTabla(nombre);
                    ParametrosPanel.actualizarPams("grupo");
                } else {
    ...
    public void llenarGruposVulnerables() {
        DefaultTableModel modelo = (DefaultTableModel) tabla.getModel();
        modelo.setRowCount(0);
        colores.clear();
        colores = (ArrayList) new GrupoVulnerable().Listar("").clone();
        for (GrupoVulnerable gv : colores) {
            modelo.addRow(new Object[]{
                gv.getId(), gv.getNombre(), gv.getEstado()
            });
        }
        if (colores.size() < 1) {
            modelo.addRow(new Object[]{
    ...
    public void llenarGruposVulnerables(String estadox) {
        DefaultTableModel modelo = (DefaultTableModel) tabla.getModel();
        modelo.setRowCount(0);
        colores.clear();
        colores = (ArrayList) new GrupoVulnerable().Listar(estadox).clone();
        for (GrupoVulnerable gv : colores) {
            if (gv.getNombre().contains(estadox)) {
                modelo.addRow(new Object[]{
                    gv.getId(), gv.getNombre(), gv.getEstado()
                });
            }
