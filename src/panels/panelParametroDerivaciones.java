 ...
 * @author Family
 */
public class panelParametroDerivaciones extends javax.swing.JPanel {

    ArrayList<Derivacion> derivaciones = new ArrayList();
    int valor = -1;
    String estado = "Activo";
    public boolean agregar = true;
    public boolean editar = true;

    public panelParametroDerivaciones() {
        initComponents();
        reset();
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Gestión de Derivaciones de Pacientes");
        ...
        nuevoBtn.setToolTipText("Agregar Cargo Adminsitrativo");
        ...
        editarBtn.setToolTipText("Editar Cargo Administrativo");
        ...
        resetBtn.setToolTipText("Actualizar resultados");
    ...
    private void editarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editarBtnActionPerformed
        ...
        if (fila >= 0 && !id.equals("-")) {
            ...
            for (Derivacion derivacion : derivaciones) {
                if (idint == derivacion.getId()) {
                    idTxt.setText(derivacion.getId() + "");
                    nombreTxt.setText(derivacion.getNombre());
                    if (derivacion.getEstado().equalsIgnoreCase("activo")) {
                        activoRb.setSelected(true);
                    } else {
    ...
    private void guardarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBtnActionPerformed
        if (validarCamposLlenos() == true) {
            ...
            if (valor == 0) {//nuevo
                ...
                for (Derivacion derivacion : derivaciones) {
                    if (derivacion.getNombre().equalsIgnoreCase(nombre)) {
                        repetido = true;
                    }
                }
                if (repetido == false) {
                    int respuesta = new Derivacion().newDerivacion(nombre);
                    if (respuesta > 0) {
                        new rojerusan.RSNotifyFade("¡¡Agregado!! ", "Derivación agregado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                        limpiarCampos();
                ...
                int respuesta = new Derivacion().update(id, nombre, estado);
                if (respuesta > 0) {
                    new rojerusan.RSNotifyFade("¡¡Actualizado!! ", "Derivacion actualizado con exito!", 4, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
                    limpiarCampos();
                    buscarValorTabla(nombre);
                } else {
    ...
    public void llenarCargos() {
        ...
        derivaciones.clear();
        derivaciones = (ArrayList) new Derivacion().Listar("").clone();
        for (Derivacion derivacion : derivaciones) {
            modelo.addRow(new Object[]{
                derivacion.getId(), derivacion.getNombre(), derivacion.getEstado()
            });
        }
        if (derivaciones.size() < 1) {
            modelo.addRow(new Object[]{
                "-", "Sin Resultados"
            });
        }
    ...
    public void llenarCargos(String estadox) {
        DefaultTableModel modelo = (DefaultTableModel) tabla.getModel();
        modelo.setRowCount(0);
        derivaciones.clear();
        derivaciones = (ArrayList) new Derivacion().Listar(estadox).clone();
        for (Derivacion derivacion : derivaciones) {
            if (derivacion.getNombre().contains(estadox)) {
                modelo.addRow(new Object[]{
                    derivacion.getId(), derivacion.getNombre(), derivacion.getEstado()
                });
            }
